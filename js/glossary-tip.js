 ...
 * their definition on a `data-gloss="…"` attribute (the encyclopedic card) or a
 * `data-tip="…"` attribute (the compact .is-label chip for control names).
 *
 * A single delegated listener on the document drives one shared card element, so
 * it survives the constant re-rendering of popups/cards without re-binding per
 * element (Leaflet popup DOM is dynamically inserted; delegation still catches
 * it). The card is read-only (pointer-events:none) and styled in css/app.css
 * (.glossary-tip).
 *
 * This is the site's single tooltip mechanism: I18n.glossAttr emits data-gloss
 * (not native title=), so every glossary term across the map, panels and this
 * card's popups renders here. Native `title=` is no longer emitted anywhere.
 */
(function () {
  'use strict';
